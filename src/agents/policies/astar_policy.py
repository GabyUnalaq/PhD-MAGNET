...
def astar(grid, start_world: tuple, goal_world: tuple) -> List[Tuple[float, float]] | None:
    """
    Standalone A* on a GridMap grid array.

    Parameters
    ----------
    grid : 2-D array (n_rows, n_cols), 1 = obstacle
    start_world : (x, y) world coordinates  (x = col, y = row)
    goal_world  : (x, y) world coordinates

    Returns
    -------
    List of (x, y) world cell-centre waypoints from start to goal,
    or None if no path exists.
    """
    n_rows, n_cols = grid.shape
    start = (int(start_world[1]), int(start_world[0]))   # (row, col)
    goal  = (int(goal_world[1]),  int(goal_world[0]))
